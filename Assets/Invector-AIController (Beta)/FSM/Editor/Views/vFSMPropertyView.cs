...
namespace Invector.vCharacterController.AI.FSMBehaviour
{
    public class vFSMPropertyView : FSMViewBase
    {
       ...
       // int removeIndex = -1;
        public override void UpdateView(Event e, vFSMBehaviour curGraph)
        {
           ...
           // rectoffset.y += 20;
            ...
            { 
                ...
                {
                    GUILayout.Space(5);
                    GUILayout.BeginHorizontal(viewSkin.GetStyle("NodeDefault"),GUILayout.Height(20));
                    GUILayout.Label("FSM COMPONENTS", viewSkin.GetStyle("LabelHeader"));
                    if (GUILayout.Button("+", viewSkin.GetStyle("NodeDefault"), GUILayout.Width(20),GUILayout.ExpandHeight(true)))
                    {
                    ...
                    else GUILayout.Box("NONE", viewSkin.GetStyle("NodeDefault"), GUILayout.ExpandWidth(true));
        ...
        //void AddParametersContext(vFSMBehaviourController graph)
        //{
        //    GenericMenu menu = new GenericMenu();
        //    menu.AddDisabledItem(new GUIContent("New FSM Parameter"));
        //    menu.AddSeparator("");
        //    var enumNames = Enum.GetNames(typeof(vFSMControllerParameterType));
        //    for (int i = 0; i < enumNames.Length; i++)
        //    {
        //        var name = enumNames[i];
        //        var enumValue = (vFSMControllerParameterType)Enum.Parse(typeof(vFSMControllerParameterType), name);
        //        menu.AddItem(new GUIContent(enumNames[i]), false, () => {
        //            graph.parameters.Add(new vFSMControllerParameter("New " + name,enumValue));
        //            curGraphSerialized.ApplyModifiedProperties(); });
        //    }
        //    menu.ShowAsContext();
        //}
        ...
        void AddComponentContext(vFSMBehaviour graph,Rect rect)
        {
            ...
            {
                GenericMenu menu = new GenericMenu();
                menu.AddDisabledItem(new GUIContent("New FSM Component"));
                menu.AddSeparator("");
                var possibleActions = typeof(vStateAction).FindSubClasses();
                foreach (var type in possibleActions)
                {
                    var instance = (vStateAction)ScriptableObject.CreateInstance(type.Name);
                    if(instance)
                    menu.AddItem(new GUIContent("Action/" + (instance.defaultName +" ("+ type.Name+")")), false, () => { AddAction(graph, type); if (instance) GameObject.DestroyImmediate(instance); });
                }
                ...
                foreach (var type in possibleDecisions)
                {
                    ...
                    if(instance)
                    menu.AddItem(new GUIContent("Decision/" + (instance.defaultName + " (" + type.Name + ")")), false, () => { AddTransition(graph, type); if (instance) GameObject.DestroyImmediate(instance); });
                }
