...
namespace Invector.vCharacterController.AI
{
    ...
    public abstract class vAIMotor : vHealthController, vICharacter
    {
        ...
        [SerializeField]
        protected vAIMovementSpeed movementSpeed = vAIMovementSpeed.Walking;
        ...
        [vHelpBox("Change the Update Mode of the AnimatorController to 'Animate Physics' for precise turnOnSpot movement", vHelpBoxAttribute.MessageType.Info)]
        ...
        [HideInInspector]
        public vAIMovementSpeed currentMovementSpeed;
        [HideInInspector]
        ...
        [HideInInspector]
        public bool stepUp, jumpOver, climbUp, landHigh, isRolling, isUsingLadder, lockMovement, stopMove, inTurn, isJumping, doingCustomAction;
        ...
        protected virtual void SetMovementInput(Vector3 input)
        {
            ...
            this.input = input;
            //this.input.x = Mathf.Clamp(this.input.x, -1, 1);
            //this.input.z = Mathf.Clamp(this.input.z, -1, 1);
        }
        ...
        {
            this.targetDirection = targetDirection.normalized;// input.magnitude > 0.2f ? targetDirection : Vector3.zero;
            this.input = Vector3.Lerp(this.input, input, smooth * Time.deltaTime);
            //this.input.x = Mathf.Clamp(this.input.x, -1, 1);
            //this.input.z = Mathf.Clamp(this.input.z, -1, 1);
        }
        ...
        public virtual void TurnOnSpot(Vector3 direction)
        {
            direction.y = 0f;
            input = Vector3.zero;
            if (direction.magnitude < 0.1f || (isStrafing ? strafeMagnitude : speed) > 0.1f || isRolling || customAction || lockMovement)
            {
                _turnOnSpotDirection = transform.forward;
                return;
            }
        ...
        float rotateInPlace
        {
            get
            {
                if (_turnOnSpotDirection.magnitude < 0.1f) return 0;
        ...
        protected virtual void StrafeMovement()
        {
            ...
            rotDir.y = 0;
            if (rotDir.magnitude > 0.1f && input.magnitude > 0.4f && !inTurn && !isRolling && !ragdolled && !lockMovement)
            {
                _turnOnSpotDirection = transform.forward;
            ...
            animator.SetFloat("InputMagnitude", isJumping ? 0 : strafeMagnitude, 0.2f, Time.deltaTime);
        }
        ...
        protected virtual void FreeMovement()
        {
            ...
            animator.SetFloat("InputMagnitude", isJumping ? 0 : speed, 0.2f, Time.deltaTime);

            var conditions = (!actions && !lockMovement);          
            ...
            {                
                ...
                if (!inTurn && !isRolling && speed > 0.1f && !ragdolled)
        ...
        protected virtual bool isSprinting
        {
            get
            {
                return currentMovementSpeed == vAIMovementSpeed.Sprinting;
            }
        }
        ...
        protected virtual void CheckGroundDistance()
        {
            if (_capsuleCollider != null && (_rigidbody.velocity.y > 0.1f || _rigidbody.velocity.y < -0.1f) || isJumping)
            {
                ...
                if (canExitJump && _rigidbody.velocity.y <= 0f && isGrounded)
        ...
        protected IEnumerator ResetJump()
        {
            yield return new WaitForSeconds(0.2f);
            canExitJump = true;
        }

        protected virtual Vector3 GetJumpForce(Vector3 targetPosition, float time)
        {
            // calculate vectors
            ...
            float t = time;
            float v0y = y / t + jumpHeight * Physics.gravity.magnitude * t;
            float v0xz = xz / t;
        ...
        protected virtual void AnimatorLocomotion()
        {
            ...
            if (turnOnSpotAnim)
            {
                ...
                if (inTurn && Mathf.Abs(rotateInPlace) < 10)
                {
                    ...
                    animator.SetFloat("TurnOnSpotDirection", turnOnSpotAnim && !isRolling && canMove && !actions && input.magnitude < 0.4f ? rotateInPlace : 0);
            }
        ...
        protected virtual void ActionsControl()
        {
            ...
            inTurn = turnOnSpotAnim && !underBodyInfo.IsName("Null");// && (rotateInPlace > 35 || rotateInPlace < -35 && input.magnitude < 0.2f);         
        ...
        public virtual void Stop()
        {
            ...
            currentMovementSpeed = vAIMovementSpeed.Idle;
        }
        ...
        {
            currentMovementSpeed = vAIMovementSpeed.Walking;
        }
        ...
        {
            currentMovementSpeed = vAIMovementSpeed.Running;
        }
        ...
        {
            currentMovementSpeed = vAIMovementSpeed.Sprinting;
        }
        ...
        public virtual void JumpTo(Vector3 jumpTarget)
        {
            ...
            if (!inTurn && isGrounded && !lockMovement && !actions)
            {
