...
namespace Invector
{
    ...
    {
        public vShooter.vShooterWeapon shooterWeapon;
        ...
        public void Shot()
        {
            if (shooterWeapon)
            {
                ...
                {
                    shooterWeapon.ShootEffect(hitObject.point, controller.transform);
                }
                else
                {
                    shooterWeapon.ShootEffect(controller._camera.transform.position + controller._camera.transform.forward * controller._camera.farClipPlane, controller.transform);
                }
            }
        }
    }
